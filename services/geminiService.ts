...
export const generateStoryScript = async (idea: string, template: StoryTemplate): Promise<{ title: string; pages: StoryPage[] }> => {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: `请基于以下创意为儿童绘本创作完整脚本（语言：中文）: "${idea}"。
               ...
               2. 6-8页的故事情节 (story)。
               3. 一个温馨的封底 (back)，包含结语。
               每一页需要一段简短文字和一段详细的视觉提示词（Visual Prompt）。`,
    config: {
...
export const generateStoryFromImage = async (base64Image: string, mimeType: string, template: StoryTemplate): Promise<{ title: string; pages: StoryPage[]; extractedIdea: string }> => {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        ...
        { text: `请分析这张照片并将其转化为神奇绘本脚本（语言：中文）。要求包含封面 (cover)、故事正文 (story) 和封底 (back)。遵循 ${template}。返回 JSON。` }
      ]
    },
    config: {
      responseMimeType: "application/json",
  ...
  return {
    title: data.title,
    extractedIdea: data.extractedIdea,
    pages: data.pages.map((p: any, idx: number) => ({
      ...p,
      id: generateId(),
      pageNumber: idx + 1
...
export const generateCharacterOptions = async (description: string, style: VisualStyle, referenceImageBase64?: string): Promise<string[]> => {
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
  const promptText = `A single character concept: ${description}. White background. Style: ${style}. Reference used if provided.`;
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        ...
        { text: `Illustration for: "${visualPrompt}". Main character attached. Style: ${style}. High quality, consistent colors.` }
      ]
    },
